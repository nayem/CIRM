function [cost, net_gradients] = computeNetGradient(net_weights, data, label, opts)
...
switch opts.cost_function
    ...
    case 'xentropy'
        % sometimes cost=NaN due to the case of 0*log(0). But it doesn't matter.
...
%% backprop
if opts.isGPU
    net_gradients = gpuArray.zeros(size(net_weights),'single');
else
    net_gradients = zeros(size(net_weights),'single');
end
