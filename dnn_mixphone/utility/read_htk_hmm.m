  ...
  if ~lastlinewashmm
    if length(file{x}) >= 2
      if file{x}(1:2) == '~h' | ~isempty(strmatch(upper(file{x}), '<BEGINHMM>'))
        tmp_hmm = readNextHMM(file, x);
        if tmp_hmm.is_tied;  % added by yuxuan: just clean up
            assert(isempty(tmp_hmm.gmms(1).tied_state_name));
            tmp_hmm.gmms = tmp_hmm.gmms(2:end);
        end
        hmms(nhmms) = tmp_hmm;
        nhmms = nhmms+1;
        lastlinewashmm = 1;
...
hmm.gmms = struct; % added by yuxuan, to avoid crash
x = linenum;
...
while isempty(findstr(upper(file{x}),'<TRANSP>'))
  ...
  hmm.is_tied = false;
  for n = 1:nmix
    if ~isempty(findstr(file{x}, '~s "')) % pointer to tied state
      hmm.is_tied = true;
      hmm.gmms(end+1).tied_state_name = file{x}(5:end-1);% added by yuxuan
      hmm.emission_type = 'GMM'; %added by yuxuan
      x = x+1;      
      break
    end
