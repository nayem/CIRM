function wss_dist= comp_wss(refData, procData, Fs)
% ----------------------------------------------------------------------
%
...
%     the 4 kHz bandwidth.  
%
%   Usage:  wss_dist=comp_wss(refData, procData, Fs)
%           
...
%
%  Example call:  ws =comp_wss(sp04,enhanced,Fs)
%
%  References:
%
...
% ----------------------------------------------------------------------
if nargin~=3
    fprintf('USAGE: WSS=comp_wss(cleanData, enhancedData,Fs)\n');
...
if iscell(refData)
    ...
    wss_dist  = zeros(num_elems,1);
    
    for i = 1:num_elems
        wss_dist_vec = wss(refData{i} + eps, procData{i} + eps,Fs(i));
        wss_dist_vec = sort(wss_dist_vec);
        wss_dist(i)  = mean(wss_dist_vec( 1: round( length( wss_dist_vec)*alpha)));
    end
else
    refData  = refData + eps;
    procData = procData + eps;
    
    wss_dist_vec = wss(refData, procData,Fs);
    wss_dist_vec = sort(wss_dist_vec);
