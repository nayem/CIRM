function is_mean= comp_is(cleanData, enhdData,Fs)
% ----------------------------------------------------------------------
...
%
%   Usage:  IS=comp_is(cleanData, enhdData, Fs)
%           
%         cleanData - clean input data
%         enhdData  - enhanced processed data
%         Fs        - sampling rate
%         IS        - computed Itakura Saito measure
% 
%         Note that the IS measure is limited in the range [0, 100].
%
%  Example call:  IS =comp_is(cleanData, enhdData,Fs)
%
%  
%  References:
%
...
if nargin~=3
    fprintf('USAGE: IS=comp_is(cleanData, enhdData,Fs)\n');
...
if iscell(cleanData)
    ...
    is_mean   = zeros(num_elems,1);
    
    for i = 1:num_elems
        IS_dist = is(cleanData{i}+eps, enhdData{i}+eps,Fs(i));
        IS_len  = round( length( IS_dist)* alpha);
        IS      = sort( IS_dist);
        is_mean(i) = mean( IS( 1: IS_len));
    end
else
    ...
    IS_dist = is( cleanData, enhdData,Fs);
    IS_len  = round( length( IS_dist)* alpha);
    IS      = sort( IS_dist);
