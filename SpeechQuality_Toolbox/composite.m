function [Csig,Cbak,Covl]= composite(cleanData, enhancedData, Fs)
% ----------------------------------------------------------------------
...
%   [1].
%
%   Usage:  [sig,bak,ovl]=composite(cleanData, enhancedData, Fs)
%
...
%
%  Example call:  [sig,bak,ovl] =composite(cleanData, enhancedData, Fs)
%
%
%  References:
%
...
if nargin~=3
    fprintf('USAGE: [sig,bak,ovl]=composite(cleanData, enhancedData, Fs)\n');
...
if iscell(cleanData)
    ...
    Csig = zeros(num_elems,1);
    Cbak = zeros(num_elems,1);
    Covl = zeros(num_elems,1);
    
    for i = 1:num_elems
        % -- compute the WSS measure ---
        %
        wss_dist_vec = wss( cleanData{i}+eps, enhancedData{i}+eps,Fs(i));
        wss_dist_vec = sort( wss_dist_vec);
        wss_dist     = mean( wss_dist_vec( 1: round( length( wss_dist_vec)*alpha)));
        
        % --- compute the LLR measure ---------
        %
        LLR_dist = llr( cleanData{i}+eps, enhancedData{i}+eps,Fs(i));
        LLRs     = sort(LLR_dist);
        ...
        % --- compute the SNRseg ----------------
        %
        [snr_dist, segsnr_dist]= snr( cleanData{i}+eps, enhancedData{i}+eps,Fs(i));
        snr_mean = snr_dist;
        segSNR= mean( segsnr_dist);
        
        
        % -- compute the pesq ----
        %[pesq_mos]= pesq(cleanData, enhancedData);
        [pesq_mos] = tstMeasurePesq2(cleanData{i}+eps, Fs(i), enhancedData{i}+eps, Fs(i));
        
        % --- now compute the composite measures ------------------
        %
        Csig(i) = 3.093 - 1.029*llr_mean + 0.603*pesq_mos-0.009*wss_dist;
        Csig(i) = max(1,Csig(i));  Csig(i)=min(5, Csig(i)); % limit values to [1, 5]
        Cbak(i) = 1.634 + 0.478 *pesq_mos - 0.007*wss_dist + 0.063*segSNR;
        Cbak(i) = max(1, Cbak(i)); Cbak(i)=min(5,Cbak(i)); % limit values to [1, 5]
        Covl(i) = 1.594 + 0.805*pesq_mos - 0.512*llr_mean - 0.007*wss_dist;
        Covl(i) = max(1, Covl(i)); Covl(i)=min(5, Covl(i)); % limit values to [1, 5]
    end
else
    ...
    % -- compute the WSS measure ---
    %
    wss_dist_vec = wss( cleanData, enhancedData,Fs);
    wss_dist_vec = sort( wss_dist_vec);
    wss_dist     = mean( wss_dist_vec( 1: round( length( wss_dist_vec)*alpha)));
    
    % --- compute the LLR measure ---------
    %
    LLR_dist = llr( cleanData, enhancedData,Fs);
    LLRs     = sort(LLR_dist);
    LLR_len  = round( length(LLR_dist)* alpha);
    llr_mean = mean( LLRs( 1: LLR_len));
    ...
    % --- compute the SNRseg ----------------
    %
    [snr_dist, segsnr_dist]= snr( cleanData, enhancedData,Fs);
    snr_mean = snr_dist;
    segSNR= mean( segsnr_dist);
    ...
    % -- compute the pesq ----
    %[pesq_mos]= pesq(cleanData, enhancedData);
    %[pesq_mos] = tstMeasurePesq2(cleanData, Fs, enhancedData, Fs);
    [pesq_mos] = pesq_dat(cleanData,enhancedData,Fs);
