function varargout=bss_crit_SI(varargin)
    ...
    case 'global'
        switch isempty(e_noise)
            case 1
                ...
                [SDR,SIR,SAR,SI_SAR]=bss_energy_ratios_SI(s_target,e_interf,e_artif);
                varargout{1}=10*log10(SDR); varargout{2}=10*log10(SIR); varargout{3}=10*log10(SAR); varargout{4}=10*log10(SI_SAR);
            case 0
                % Computation of the energy ratios
                [SDR,SIR,SNR,SAR,SI_SAR]=bss_energy_ratios_SI(s_target,e_interf,e_noise,e_artif);
                varargout{1}=10*log10(SDR); varargout{2}=10*log10(SIR);
                varargout{3}=10*log10(SNR); varargout{4}=10*log10(SAR); varargout{5}=10*log10(SI_SAR);               
        end
    ...
    case 'local'
        ...
        switch isempty(e_noise)
            case 1
                ...
                [SDR,SIR,SAR,SI_SAR]=bss_energy_ratios_SI(F_s_target,F_e_interf,F_e_artif);
                varargout{1}=10*log10(SDR); varargout{2}=10*log10(SIR); varargout{3}=10*log10(SAR); varargout{4}=10*log10(SI_SAR);
            case 0
                F_s_target=bss_make_frames(s_target,WINDOW,NOVERLAP);
                F_e_interf=bss_make_frames(e_interf,WINDOW,NOVERLAP);
                ...
                F_e_artif=bss_make_frames(e_artif,WINDOW,NOVERLAP);
                [SDR,SIR,SNR,SAR,SI_SAR]=bss_energy_ratios_SI(F_s_target,F_e_interf,F_e_noise,F_e_artif);
                varargout{1}=10*log10(SDR); varargout{2}=10*log10(SIR);
                varargout{3}=10*log10(SNR); varargout{4}=10*log10(SAR); varargout{5}=10*log10(SI_SAR); 
        end        
end %mode