function fwseg_dist= comp_fwseg(cleanData, enhancedData,Fs)
...
%
%   Usage:  fwSNRseg=comp_fwseg(cleanData, enhancedData,Fs)
%           
%         cleanData    - clean input file in .wav format
%         enhancedData - enhanced output file in .wav format
%         Fs           - sampling rate
%         fwSNRseg     - computed frequency weighted SNRseg in dB
% 
%         Note that large numbers of fwSNRseg are better.
%
%  Example call:  fwSNRseg =comp_fwseg(cleanData, enhancedData,Fs)
%
%  
%  References:
...
if nargin~=3
    fprintf('USAGE: fwSNRseg=comp_fwseg(cleanData, enhancedData,Fs)\n');
...
if iscell(cleanData)
    ...
    fwseg_dist = zeros(num_elems,1);
    
    for i = 1:num_elems
        wss_dist_vec  = fwseg(cleanData{i}+eps, enhancedData{i}+eps,Fs(i));
        fwseg_dist(i) = mean(wss_dist_vec);
    end
else
    ...
    wss_dist_vec= fwseg( cleanData, enhancedData,Fs);
