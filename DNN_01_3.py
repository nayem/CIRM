...
def calc(x, y):
    ...
    # mse_r = tf.losses.mean_squared_error(labels=y[:, :c//2], predictions=predictions[:,:c//2])
    # mse_i = tf.losses.mean_squared_error(labels=y[:, c//2:], predictions=predictions[:, c//2:])
    # loss_d = tf.divide( (-tf.reduce_sum(mse_r)-tf.reduce_sum(mse_i)), r)

    return [predictions, loss_t, loss_d ]
...
for epoch in range(opts.sgd_max_epoch):
    ...
    for batch_num, (batch_x, batch_y) in enumerate(opts.next_batch(opts.cvData.shape[0], opts.sgd_batch_size, isTrainCycle=False)):
        ...
        c = sess.run(loss_cv, feed_dict={X: batch_x, Y: batch_y})
        avg_cost += c/(batch_num+1)

        if batch_num % 20 == 0:
            print('[D] - Epoch:', epoch, ', batch_num:', batch_num, ", cost:", c , ",Avg Cost:", avg_cost)

    print('[D] - Epoch:', epoch, ",Avg Cost:", avg_cost)
