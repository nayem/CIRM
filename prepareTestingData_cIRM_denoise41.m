function [teData, teLabel_r,teLabel_i, para, numframes] = prepareTestingData_cIRM_denoise41(STATS_FILE,globalpara,cs_testing_data_path,testing_mix_wav_save_path)
...
% ------------------------ Initialize data -------------------------------%
teData=0; teLabel_r=0; teLabel_i=0; para=0;
...
teFilename  = strings(num_files,1);
...
clearvars -except STATS_FILE para arma_order feawin teData teLabel_r teLabel_i numframes spec_clean_r spec_clean_i spec_mixture_r spec_mixture_i teFilename cleanFilename
...
%     teData_t = teData;  
%     teLabel_r_t = teLabel_r;    
%     teLabel_i_t = teLabel_i;
    numframes_t = numframes;    
    teFilename_t = teFilename;
...
%     load(STATS_FILE, 'teData','teLabel_r','teLabel_i','numframes','teFilename','cleanFilename','spec_clean_r','spec_clean_i','spec_mixture_r','spec_mixture_i','uncomp_mixture_label_r','uncomp_mixture_label_i');
    load(STATS_FILE, 'numframes','teFilename','cleanFilename','spec_clean_r','spec_clean_i','spec_mixture_r','spec_mixture_i');

%     teData = [teData;teData_t];  
%     teLabel_r = [teLabel_r;teLabel_r_t];    
%     teLabel_i = [teLabel_i;teLabel_i_t];
    numframes = [numframes;numframes_t];    
    teFilename = [teFilename;teFilename_t];
...
save(STATS_FILE, 'numframes','teFilename','cleanFilename','spec_clean_r','spec_clean_i','spec_mixture_r','spec_mixture_i', '-v7.3');
