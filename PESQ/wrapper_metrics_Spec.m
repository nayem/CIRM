function wrapper_metrics_Spec(Version,noise_type,metrics)
...
% Currently, it works only for single NOISE TYPE ****
...
%         metrics = {,'PESQ','OVRL','CCD','SRR_f','SNR','SNR_SEG','STOI','BSS_SDR','BSS_SIR','BSS_SAR','PHASE'};
        metrics = {'REV-PESQ','PESQ','REV-STOI','STOI','REV-BSS_SDR','BSS_SDR','REV-SI_SDR','SI_SDR','OVRL','CCD','SRR_f','SNR','SNR_SEG','BSS_SIR','BSS_SAR','PHASE'};
    else
        metrics = {metrics};
    end

    total_scores_list = load(sprintf('../scores/SPECscores_denoising.noise%s%s.mat',noise_type,Version));
    ...
    for m=1:length(metrics)
        ...
        if strcmpi(metric,'REV-PESQ')   % PESQ
            var = 'pesq_rev';
        elseif strcmpi(metric,'PESQ')   % PESQ
            var = 'pesq_derev';
        ...
        elseif strcmpi(metric,'SNR_SEG')    % SNR_SEG
            var = 'segsnr_derev';
        elseif strcmpi(metric,'REV-STOI')   % STOI
            var = 'stoi_rev';
        elseif strcmpi(metric,'STOI')   % STOI
            var = 'stoi_derev';
        elseif strcmpi(metric,'REV-BSS_SDR')   % BSS_SDR
            var = 'SDR_rev';
        elseif strcmpi(metric,'BSS_SDR')   % BSS_SDR
            var = 'SDR_derev';
        elseif strcmpi(metric,'REV-SI_SDR')   % BSS_SDR
            var = 'SI_SDR_rev';
        elseif strcmpi(metric,'SI_SDR')   % BSS_SDR
            var = 'SI_SDR_derev';
