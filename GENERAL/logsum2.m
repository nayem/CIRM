...
s=size(x);
p=[d1,d2:ndims(x),1:d1-1];
z=reshape(permute(x,p),n1,n2,prod(s)/(n1*n2));
q=max(max(z,[],1),[],2);              % we subtract y from each row to avoid dynamic range problems
a=(q==Inf)|(q==-Inf);       % check for infinities
if nargin<4
    y=q+log(sum(sum(exp(z-q(ones(n1,1),ones(1,n2),:)),1),2));
elseif numel(k)==n1 % Need to update for logsum2
    y=q+log(sum(exp(z-q(ones(n1,1),:)).*repmat(k(:),1,prod(s)/n1),1));
else % Need to update for logsum2
