function scriptTestDNN_cIRM_denoise()
...
ENHANCED_PHRASE = 'crmenh_tf';
% mix_wavs_data_path = '/data/knayem/denoise_complex_domain_wavs/';
...
%     net_file = sprintf('./dnn_models/dnncirm.noise%s.mat',noise_types{noise_ind});
    net_file = sprintf('./dnn_models/DNN_CIRM_net.mat');

    testing_clean_wav_save_path       = sprintf('/data/knayem/denoising_clean_wavs_SSN_10noisespercs/testing_16k/',noise_types{noise_ind});
    testing_matched_mix_wav_save_path = sprintf('/data/knayem/denoising_mix_wavs_SSN_10noisespercs/testing_matched/',noise_types{noise_ind});
...
%         CLEAN_FILENAME   = sprintf('%sclean%02d_%dkHz.wav',testing_clean_wav_save_path,str2double(tmpe(4:end)),Fs/1000);
        ...
        figure('Name','Complex Mask');
        plot(complex_irmmask);
        title('Complex Mask');
        xlabel('x');
        ylabel('y');
        axis tight;
        ...
        filename    = sprintf('%s%s_%s.%s',mix_wavs_data_path,f(1),ENHANCED_PHRASE,f(2));%cs_count,rem);
%         Nayem edit, Sep 12
